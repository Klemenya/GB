...
int isBalanced(TreeNode *root, int num) {
    ...
    depthCount(root->left);
    countLeft = heightCount;
    heightCount = 0;
    tempCount = 1;

    depthCount(root->right);
    countRight = heightCount;
    heightCount = 0;
    tempCount = 1;

    if (countLeft == countRight || countLeft == countRight + 1 || countLeft == countRight - 1) {

        printf("Tree %2d is balansed tree.\t Left: %d, right: %d\n", num + 1, countLeft, countRight);
        return 1;
    } else {
        printf("Tree %2d is unbalansed tree.\t Left: %d, right: %d\n", num + 1, countLeft, countRight);
        return 0;
    }
}
